...
from utils_coords import project_verts
...
class MeshRefinementHead(nn.Module):
    def __init__(self):
        ...
        # fmt: off
        input_channels  = 3840 # sum of channels from conv2_3, conv3_4, conv4_6, conv5_3
        self.num_stages = 3
        hidden_dim      = 128
        stage_depth     = 3
...
class MeshRefinementStage(nn.Module):
    ...
    def forward(self, img_feats, meshes, vert_feats=None, P=None):
        ...
        # Texture (trial)
        textures = torch.sigmoid(self.vert_offset(vert_feats))
        textures_list = [textures]
        meshes_out.textures = TexturesVertex(verts_features=textures_list)
